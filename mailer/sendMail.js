...
const sendOnboardingMail = (data) => {
    var transporter = nodemailer.createTransport({
      ...
      let bookingData = `
                    ...
                    ${data.data.map((el) => {
                        return(`<tr>
                            <td style="border: 1px solid #999; padding: 0.5rem;text-align: left;">${el.category}</td>
                            <td style="border: 1px solid #999; padding: 0.5rem;text-align: left;">${el.ticket_count}</td>
                            <td style="border: 1px solid #999; padding: 0.5rem;text-align: left;">₹${Number(el.amount)} x ${el.ticket_count} = ₹${el.ticket_count * Number(el.amount)}</td>
                        </tr>`)
                    })}
                    </table>
